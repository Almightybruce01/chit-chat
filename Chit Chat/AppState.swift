...
final class AppState: ObservableObject {
    ...
    @Published var broadcastChannels: [BroadcastChannel] = [
        BroadcastChannel(id: UUID(), title: "Chit Chat Updates", ownerHandle: "@chitchat", memberCount: 12024, latestMessage: "Live shopping upgrades shipped."),
    ...
    func publishPost(
        ...
        case .safe, .violenceNeedsConsent:
            ...
            newPost = created
        case .blockedNudity, .manualReview:
            break
        }
    ...
    func requestVerification(note: String, hasInstagramVerification: Bool) {
    ...
    func setProfileQuote(_ text: String) {
        currentUser.profileQuote = text.trimmingCharacters(in: .whitespacesAndNewlines)
        syncCurrentUserInDirectory()
    ...
    func addContract(title: String, budgetUSD: Int, location: String, isLocalHire: Bool) {
    ...
    func syncContacts(displayNames: [String], identifiers: [String]) {
        ...
        contactsSyncStatus = matches.isEmpty
            ? "No matching contacts found in Chit Chat yet."
            : "Found \(matches.count) contacts on Chit Chat."
    }
    ...
    func repostPost(_ postID: UUID) {
        ...
        var firstTimeRepost = false
        if let sourceIndex = posts.firstIndex(where: { $0.id == postID }) {
            let current = engagementUser(for: currentUser.handle)
            ...
            if !alreadyReposted {
                ...
                firstTimeRepost = true
            }
        }
        let reposted = PostItem(
            id: UUID(),
            authorHandle: currentUser.handle,
            caption: firstTimeRepost
    ...
    func replyToPost(_ postID: UUID, text: String) {
        let trimmed = text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmed.isEmpty else { return }
        _ = addComment(to: postID, text: trimmed)
        if let source = posts.first(where: { $0.id == postID }) {
            let replyPost = PostItem(
                id: UUID(),
                authorHandle: currentUser.handle,
                caption: "Replying to \(source.authorHandle): \(trimmed)",
                type: .post,
                createdAt: Date(),
                city: localCity,
                imageData: nil,
