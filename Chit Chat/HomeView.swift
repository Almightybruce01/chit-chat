...
struct ReelsView: View {
        ...
        GeometryReader { proxy in
            ZStack {
                ...
                TabView(selection: $selectedIndex) {
                    ForEach(Array(safeReelItems.enumerated()), id: \.offset) { index, item in
    ...
    private var reelItems: [ReelDemoItem] {
        let source = appState.posts.filter { $0.type == .reel || $0.type == .shortVideo || $0.type == .story }
        if !source.isEmpty {
            return source.enumerated().map { idx, post in
                ReelDemoItem(
                    id: post.id,
                    authorHandle: post.authorHandle,
                    caption: post.caption,
                    audioTitle: "AI Beat Mix",
                    likeCount: max(post.likeCount, Int.random(in: 1400...24000)),
                    commentCount: max(post.commentCount, Int.random(in: 120...3200)),
                    shareCount: max(post.repostCount, Int.random(in: 30...900)),
                    imageData: post.imageData ?? appState.generatedMediaImageData(seed: "\(post.authorHandle)-\(post.id.uuidString)", isReel: true),
                    remoteImageURL: demoPhotoURL(for: post.id.uuidString, index: idx),
                    remoteVideoURL: demoVideoURL(index: idx)
                )
            }
        }

        let fallbackHandles = ["@creatorone", "@pixelvibe", "@futurelabs", "@citypulse", "@trendforge", "@neonstudio", "@alphaframe", "@viralcore"]
        let fallbackCaptions = [
            "AI-edited skyline timelapse in 4K. Night mode hits different.",
            "One-tap outfit switch with gen-AI style transfer.",
            "Short film cut from text prompt to final scene.",
            "Street food color-grade challenge. Before/after in 12 seconds.",
            "Product drop teaser auto-cut by creator assistant.",
            "Motion poster generated from one selfie + vibe prompt.",
            "Creator collab remix with dynamic beat sync.",
            "Hyperlapse city rush edited in under a minute."
        ]
        let fallbackAudio = [
            "original audio - creatorone",
            "future bounce - ai.mix",
            "night drive loop - neon",
            "cinematic pulse - studio",
            "drop alert fx - trendforge",
            "city rhythm - alpha",
            "retro wave - collab",
            "tempo shift - viralcore"
        ]

        return (0..<fallbackHandles.count).map { idx in
            ReelDemoItem(
                id: UUID(),
                authorHandle: fallbackHandles[idx],
                caption: fallbackCaptions[idx],
                audioTitle: fallbackAudio[idx],
                likeCount: Int.random(in: 2000...38000),
                commentCount: Int.random(in: 80...4200),
                shareCount: Int.random(in: 15...1400),
                imageData: appState.generatedMediaImageData(seed: "\(fallbackHandles[idx])-\(idx)", isReel: true),
                remoteImageURL: demoPhotoURL(for: fallbackHandles[idx], index: idx),
                remoteVideoURL: demoVideoURL(index: idx)
            )
        }
    }
    ...
    private func reelBackground(for item: ReelDemoItem) -> some View {
        if let remoteVideoURL = item.remoteVideoURL {
            LoopingVideoView(url: remoteVideoURL)
                .ignoresSafeArea()
    ...
    private func demoPhotoURL(for seed: String, index: Int) -> URL? {
        let query = index % 2 == 0 ? "animal,pet" : "character,cosplay"
        return URL(string: "https://loremflickr.com/720/1280/\(query)?lock=\(abs(seed.hashValue % 99999))")
    }

    private func demoVideoURL(index: Int) -> URL? {
        let urls = [
