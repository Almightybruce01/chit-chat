                ...
                VStack(spacing: 14) {
                    ...
                    sectionCard(title: "Approvals Center", icon: "checkmark.seal.text.page.fill") {
                        ...
                        NavigationLink(destination: AdminDashboardView().environmentObject(appState)) {
                            Label("Open Chit Chat Social admin", systemImage: "person.badge.shield.checkmark")
                        }
                        .buttonStyle(.borderedProminent)
                    }
                    ...
                    if !appState.currentUser.isBusinessAccount {
                        sectionCard(title: "Business verification", icon: "checkmark.seal.fill") {
                            Text("Verify your business to post jobs. Only business accounts can post jobs; each post requires approval.")
                                .font(.caption)
                                .foregroundStyle(secondaryText)
                            NavigationLink(destination: VerificationView(initialCategory: .business).environmentObject(appState)) {
                                Label("Request business verification", systemImage: "briefcase.badge.person.crop")
                            }
                            .buttonStyle(.borderedProminent)
                        }
                    }
                    ...
                    sectionCard(title: "Jobs", icon: "briefcase.fill") {
                        if !appState.currentUser.isBusinessAccount {
                            Text("Verify your business above to post jobs.")
                                .font(.caption)
                                .foregroundStyle(secondaryText)
                        } else if appState.contracts.filter({ $0.authorHandle == appState.currentUser.handle }).isEmpty {
                            Text("No opportunities yet.")
                                .foregroundStyle(secondaryText)
                        } else {
                            ForEach(appState.contracts.filter { $0.authorHandle == appState.currentUser.handle }.prefix(5)) { contract in
                                HStack {
