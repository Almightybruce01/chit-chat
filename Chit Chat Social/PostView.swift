...
struct PostView: View {
    ...
    @State private var contractStatus = ""
    ...
    private var publishBlockedReason: String? {
        ...
        case .reel, .shortVideo:
            if selectedVideoData == nil || selectedVideoData?.isEmpty == true {
                return "Add a video to publish a reel or short clip."
            }
            return nil
        case .story, .post:
            return nil
    ...
    private var canPublish: Bool { publishBlockedReason == nil }

    private var mediaPickerFilter: PHPickerFilter {
        switch selectedType {
        case .reel, .shortVideo:
            return .videos
        case .story, .post:
                    ...
                    Form {
                        Section("Create Contract") {
                            ...
                            Button("Publish Contract") {
                                ...
                                let (_, msg) = appState.addContract(
    ...
    private var mediaStepCard: some View {
        EliteSectionCard {
            VStack(alignment: .leading, spacing: 12) {
                ...
                PhotosPicker(selection: $selectedMediaItem, matching: mediaPickerFilter) {
                    ZStack {
                        ...
                        if let selectedPhotoData, let image = UIImage(data: selectedPhotoData) {
                            ...
                            VStack(spacing: 8) {
                                ...
                                Text(selectedType == .reel ? "Reels need a vertical video." : "Reels + stories can be video.")
                                    .font(.caption)
    ...
    private func publishPost() {
        ...
        if let publishBlockedReason {
            publishStatus = publishBlockedReason
            return
        }
        Task {
            isUploading = true
            ...
            let videoForPublish: Data? = {
                switch selectedType {
                case .reel, .shortVideo, .story:
